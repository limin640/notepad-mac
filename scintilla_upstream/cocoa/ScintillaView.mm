...
- (NSAttributedString *) attributedSubstringForProposedRange: (NSRange) aRange actualRange: (NSRangePointer) actualRange {
	...
	// SCI_GETSTYLEINDEXAT reports a signed byte but want an unsigned to index into styles
	const char styleByte = static_cast<char>([mOwner message: SCI_GETSTYLEINDEXAT wParam: posRange.location]);
...
- (void) cut: (id) sender {
#pragma unused(sender)
	mOwner.backend->Cut(false, false);
}

- (void) copy: (id) sender {
#pragma unused(sender)
	mOwner.backend->Copy(false);
}
