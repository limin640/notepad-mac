...
bool SurfaceImpl::Initialised() const noexcept {
...
void SurfaceImpl::Init(WindowID) noexcept {
...
void SurfaceImpl::Init(SurfaceID sid, WindowID, bool) noexcept {
	Release();
...
void SurfaceImpl::SetRenderingParams(void *, void *) noexcept {
	// macOS: no DirectWrite-style rendering params
}

void SurfaceImpl::SetMode(SurfaceMode mode_) noexcept {
	mode = mode_;
}

//--------------------------------------------------------------------------------------------------

bool SurfaceImpl::SupportsFeature(Supports feature) const noexcept {
 ...
 */
int SurfaceImpl::LogPixelsY() const noexcept {
	return 72;
}
 ...
 */
int SurfaceImpl::PixelDivisions() const noexcept {
	if (gc) {
 ...
 */
int SurfaceImpl::DeviceHeightFont(int points) const noexcept {
	return points;
}

//--------------------------------------------------------------------------------------------------

void SCICALL SurfaceImpl::LineDraw(Point start, Point end, Stroke stroke) {
...
void SCICALL SurfaceImpl::PolyLine(const Point *pts, size_t npts, Stroke stroke) {
...
void SCICALL SurfaceImpl::Polygon(const Scintilla::Internal::Point *pts, size_t npts, FillStroke fillStroke) {
...
void SCICALL SurfaceImpl::RectangleDraw(PRectangle rc, FillStroke fillStroke) {
	if (!gc)
		return;
	CGContextBeginPath(gc);
	SetFillStroke(fillStroke);
...
void SCICALL SurfaceImpl::RectangleFrame(PRectangle rc, Stroke stroke) {
	if (!gc)
		return;
...
void SCICALL SurfaceImpl::FillRectangle(PRectangle rc, Fill fill) {
	if (gc) {
...
void SCICALL SurfaceImpl::FillRectangleAligned(PRectangle rc, Fill fill) {
...
void SCICALL SurfaceImpl::FillRectangle(PRectangle rc, Surface &surfacePattern) {
...
void SCICALL SurfaceImpl::RoundedRectangle(PRectangle rc, FillStroke fillStroke) {
...
void SCICALL Scintilla::Internal::SurfaceImpl::AlphaRectangle(PRectangle rc, XYPOSITION cornerSize, FillStroke fillStroke) {
	if (gc) {
...
void SCICALL Scintilla::Internal::SurfaceImpl::GradientRectangle(PRectangle rc, const std::vector<ColourStop> &stops, GradientOptions options) {
	if (!gc) {
		return;
	}
...
void SCICALL SurfaceImpl::DrawRGBAImage(PRectangle rc, int width, int height, const unsigned char *pixelsImage) {
...
void SCICALL SurfaceImpl::Ellipse(PRectangle rc, FillStroke fillStroke) {
...
void SCICALL SurfaceImpl::Stadium(PRectangle rc, FillStroke fillStroke, Ends ends) {
...
void SCICALL SurfaceImpl::Copy(PRectangle rc, Scintilla::Internal::Point from, Surface &surfaceSource) {
...
static XYPOSITION QuartzAscent(const Font *font_) {
	const QuartzTextStyle *style = TextStyleFromFont(font_);
	if (!style) {
		return 1;
	}
...
static XYPOSITION QuartzDescent(const Font *font_) {
	const QuartzTextStyle *style = TextStyleFromFont(font_);
	if (!style) {
		return 1;
	}
...
// Height/AverageCharWidth folded into Metrics()


FontMetrics SurfaceImpl::Metrics(const Font *font_) noexcept {
	const QuartzTextStyle *style = TextStyleFromFont(font_);
	if (!style) {
		return {1, 1, 0, 2};
	}
	const CGFloat ascent = style->getAscent();
	const CGFloat descent = style->getDescent();
	const XYPOSITION asc = static_cast<XYPOSITION>(ascent);
	const XYPOSITION desc = static_cast<XYPOSITION>(descent);
	return {asc, desc, 0, asc + desc};
}

void SCICALL SurfaceImpl::SetClip(PRectangle rc) noexcept {
	CGContextSaveGState(gc);
	CGContextClipToRect(gc, PRectangleToCGRect(rc));
}

void SurfaceImpl::PopClip() noexcept {
	CGContextRestoreGState(gc);
}

void SurfaceImpl::FlushCachedState() noexcept {
	CGContextSynchronize(gc);
}

void SurfaceImpl::FlushDrawing() noexcept {
}
...
// Window::~Window() noexcept = default;  (defined in header)
...
void SCICALL Window::SetPosition(PRectangle rc) noexcept {
	if (wid) {
		id idWin = (__bridge id)(wid);
		if ([idWin isKindOfClass: [NSView class]]) {
			// NSView
...
void SCICALL Window::SetPositionRelative(PRectangle rc, const Window *relativeTo) noexcept {
	PRectangle rcOther = relativeTo->GetPosition();
 ...
 */
void SCICALL Window::InvalidateRectangle(PRectangle rc) noexcept {
	if (wid) {
		id idWin = (__bridge id)(wid);
		NSView *container;
		if ([idWin isKindOfClass: [NSView class]]) {
...
PRectangle SCICALL Window::GetMonitorRect(Point) const noexcept {
	if (wid) {
		id idWin = (__bridge id)(wid);
		if ([idWin isKindOfClass: [NSView class]]) {
			NSView *view = idWin;
...
public:
	...
	void SCICALL Create(Window &parent, int ctrlID, Scintilla::Internal::Point location, int lineHeight_, int codePage_, Scintilla::Technology technology_) noexcept override;
	...
	void Append(const char *s, int type = -1);
...
void SCICALL ListBoxImpl::Create(Window & /*parent*/, int /*ctrlID*/, Scintilla::Internal::Point pt,
			 int lineHeight_, int codePage_, Scintilla::Technology technology_) noexcept {
	lineHeight = lineHeight_;
	unicodeMode = codePage_ == SC_CP_UTF8;
	maxWidth = 2000;
...
void ListBoxImpl::SetFont(std::shared_ptr<Font> font_) noexcept {
	// NSCell setFont takes an NSFont* rather than a CTFontRef but they
	// are the same thing toll-free bridged.
	QuartzTextStyle *style = TextStyleFromFont(font_.get());
...
void ListBoxImpl::SetAverageCharWidth(int width, double) noexcept {
	aveCharWidth = width;
}

void ListBoxImpl::SetVisibleRows(int rows) noexcept {
	desiredVisibleRows = rows;
}

int ListBoxImpl::GetVisibleRows() const noexcept {
...
int ListBoxImpl::CaretFromEdge() const noexcept {
	if (colIcon.hidden)
		return 3;
	else
...
void ListBoxImpl::Append(const char *s, int type) {
	int count = Length();
	{
		std::vector<char> buf(s, s + strlen(s) + 1);
		ld.Add(count, type, buf.data());
	}
...
void SCICALL ListBoxImpl::SetOptions(const ListOptions &) noexcept {
}

int ListBoxImpl::Length() const noexcept {
	return ld.Length();
}
...
int ListBoxImpl::GetSelection() const noexcept {
	return static_cast<int>(table.selectedRow);
}

std::string ListBoxImpl::GetValue(int n) const {
...
void ListBoxImpl::ClearRegisteredImages() noexcept {
...
void SCICALL Menu::Show(Point, const Window &) noexcept {
