...
private:
	...
	bool GetPasteboardData(NSPasteboard *board, SelectionText *selectedText) const;
	void SetPasteboardData(NSPasteboard *board, const SelectionText &selectedText) const;
...
protected:
	...
	PRectangle GetClientDrawingRectangle() const noexcept override;
	Point ConvertPoint(NSPoint point);
	void SCICALL RedrawRect(PRectangle rc) noexcept override;
	void DiscardOverdraw() noexcept override;
...
public:
	...
	void NotifyParent(Scintilla::NotificationData &scn) const noexcept override;
	...
	void AddToPopUp(const char *label, int cmd = 0, bool enabled = true) const noexcept;
	void ClaimSelection() const noexcept;
	...
	void DeleteBackward();
	void Cut(bool asBinary, bool lineCopy = false) override;
	void Undo() override;
	void Redo() override;
