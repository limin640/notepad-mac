 ...
 */
void ScintillaCocoa::Finalise() noexcept {
	ObserverRemove();
...
void ScintillaCocoa::QueueIdleWork(WorkItems items, Sci::Position upTo) noexcept {
...
std::unique_ptr<CaseFolder> ScintillaCocoa::CaseFolderForEncoding() const {
 ...
 */
std::string ScintillaCocoa::CaseMapString(const std::string &s, CaseMapping caseMapping) const {
 ...
 */
void ScintillaCocoa::CancelModes() noexcept {
 ...
 */
Scintilla::Internal::Point ScintillaCocoa::GetVisibleOriginInMain() const noexcept {
 ...
 */
PRectangle ScintillaCocoa::GetClientRectangle() const noexcept {
	NSScrollView *scrollView = ScrollContainer();
 ...
 */
PRectangle ScintillaCocoa::GetClientDrawingRectangle() const noexcept {
#if MAC_OS_X_VERSION_MAX_ALLOWED > 1080
	NSView *content = (__bridge NSView *)(wMain.GetID());
 ...
 */
void SCICALL ScintillaCocoa::RedrawRect(PRectangle rc) noexcept {
	if (!rc.Empty())
...
void ScintillaCocoa::DiscardOverdraw() noexcept {
#if MAC_OS_X_VERSION_MAX_ALLOWED > 1080
 ...
 */
void ScintillaCocoa::Redraw() noexcept {
 ...
 */
bool ScintillaCocoa::FineTickerRunning(TickReason reason) const noexcept {
 ...
 */
void ScintillaCocoa::FineTickerStart(TickReason reason, int millis, int tolerance) noexcept {
 ...
 */
void ScintillaCocoa::FineTickerCancel(TickReason reason) noexcept {
...
bool ScintillaCocoa::SetIdle(bool on) noexcept {
	if (idler.state != on) {
		idler.state = on;
		if (idler.state) {
...
void ScintillaCocoa::CopyToClipboard(const SelectionText &selectedText) const {
	SetPasteboardData([NSPasteboard generalPasteboard], selectedText);
}

//--------------------------------------------------------------------------------------------------

void ScintillaCocoa::Copy(bool asBinary) const {
	if (!sel.Empty()) {
...
bool ScintillaCocoa::CanPaste() const noexcept {
	if (!Editor::CanPaste())
		return false;
...
void SCICALL ScintillaCocoa::CreateCallTipWindow(PRectangle rc) noexcept {
...
void ScintillaCocoa::AddToPopUp(const char *label, int cmd, bool enabled) const noexcept {
	NSMenuItem *item;
	// macOS port: fork disables ScintillaBase popup; use the context menu from CreateContextMenu
	ScintillaContextMenu *menu = (__bridge ScintillaContextMenu *)(ct.wCallTip.GetID());
	if (menu == nil) {
		menu = [[ScintillaContextMenu alloc] initWithTitle: @""];
	}
	[menu setOwner: const_cast<ScintillaCocoa *>(this)];
...
void ScintillaCocoa::ClaimSelection() const noexcept {
...
- (id) initWithSelectedText: (const SelectionText *) other {
	...
	if (self) {
		selectedText.Copy(std::string(other->Data(), other->Length()), other->codePage, other->rectangular, other->lineCopy);
		selectedText.asBinary = other->asBinary;
	}
...
#pragma unused(item)
	...
	CFStringEncoding encoding = EncodingFromCharacterSet(selectedText.codePage == SC_CP_UTF8, Scintilla::CharacterSet::Ansi);
...
void ScintillaCocoa::SetPasteboardData(NSPasteboard *board, const SelectionText &selectedText) const {
	if (selectedText.Length() == 0)
		return;
 ...
 */
bool ScintillaCocoa::GetPasteboardData(NSPasteboard *board, SelectionText *selectedText) const {
	...
	if (data != nil) {
		if (selectedText != nil) {
			...
			selectedText->Copy(dest, pdoc->dbcsCodePage, rectangular, false);
		}
		return true;
	}
...
void ScintillaCocoa::SetMouseCapture(bool on) noexcept {
	capturedMouse = on;
}

//--------------------------------------------------------------------------------------------------

bool ScintillaCocoa::HaveMouseCapture() const noexcept {
...
void ScintillaCocoa::NotifyChange() const noexcept {
	if (notifyProc != NULL) {
		const auto wParam = static_cast<uintptr_t>(static_cast<unsigned short>(GetCtrlID())) |
			(static_cast<uintptr_t>(static_cast<unsigned short>(FocusChange::Change)) << 16);
		notifyProc(notifyObj, WM_COMMAND, wParam, (uintptr_t) this);
	}
}

//--------------------------------------------------------------------------------------------------

bool ScintillaCocoa::ValidCodePage(int codePage) const noexcept {
	// accept UTF-8, no-codepage (8-bit) and the DBCS codepages handled by CoreFoundation
	return codePage == 0 || codePage == SC_CP_UTF8
		|| codePage == 932 || codePage == 936 || codePage == 949 || codePage == 950;
}

void ScintillaCocoa::UpdateSystemCaret() {
	// macOS: caret is drawn by the view itself; no system caret to update.
}

void ScintillaCocoa::NotifyCaretMove() const noexcept {
	// macOS port: caret move handled via standard notifications; nothing extra needed.
}

void ScintillaCocoa::NotifyFocus(bool focus) const noexcept {
	if (commandEvents && notifyProc) {
		const auto wParam = static_cast<uintptr_t>(static_cast<unsigned short>(GetCtrlID())) |
			(static_cast<uintptr_t>(static_cast<unsigned short>(focus ? FocusChange::Setfocus : FocusChange::Killfocus)) << 16);
		notifyProc(notifyObj, WM_COMMAND, wParam, (uintptr_t) this);
	}
 ...
 */
void ScintillaCocoa::NotifyParent(NotificationData &scn) const noexcept {
...
void ScintillaCocoa::Cut(bool asBinary, bool lineCopy) {
	Editor::Cut(asBinary, lineCopy);
}
...
bool ScintillaCocoa::ShouldDisplayPopupOnMargin() {
	return false; // popup disabled in fork
}

bool ScintillaCocoa::ShouldDisplayPopupOnText() {
	return false; // popup disabled in fork (SCI_EnablePopupMenu=0)
}
...
NSMenu *ScintillaCocoa::CreateContextMenu(NSEvent * /* event */) {
	// macOS port: ScintillaBase popup is compiled out (SCI_EnablePopupMenu=0).
	// Context menu is provided by the host app layer if needed.
	return nil;
}
...
void ScintillaCocoa::HandleCommand(NSInteger command) {
	// macOS port: menu command routing disabled with popup; forward as WndProc command.
	WndProc(static_cast<Message>(command), 0, 0);
}
 ...
 */
void ScintillaCocoa::UpdateBaseElements() noexcept {
	NSView *content = ContentView();
...
void ScintillaCocoa::ShowFindIndicatorForRange(NSRange charRange, BOOL retaining) {
	...
	if (charRange.length) {
		...
		const char styleByte = static_cast<char>(WndProc(Message::GetStyleIndexAt, charRange.location, 0));
