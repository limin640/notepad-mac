...
void Document::ExtractCharacter(Sci::Position position, CharacterWideInfo &charInfo) const noexcept {
	...
	if (UTF8IsAscii(leadByte)) {
		...
		unsigned len = 0;
#if defined(_WIN32)
		len = ::MultiByteToWideChar(dbcsCodePage, 0, charBytes, widthCharBytes, charInfo.buffer, 2);
#else
		{
			const std::wstring ws = WStringFromMultiByte(dbcsCodePage, charBytes, widthCharBytes);
			if (!ws.empty()) {
				len = std::min<unsigned>(ws.size(), 2);
				std::copy_n(ws.begin(), len, charInfo.buffer);
			}
		}
#endif
		if (len == 0) {
...
std::wstring WStringFromMultiByte(int codePage, const char *pattern, size_t patternLen) {
	...
	return ws;
#else
	// macOS port: Windows codepage -> CFStringEncoding -> UTF-16
	CFStringEncoding encoding = kCFStringEncodingUTF8;
	switch (codePage) {
	case 932: encoding = kCFStringEncodingShiftJIS; break;
	case 936: encoding = kCFStringEncodingGB_18030_2000; break;
	case 949: encoding = kCFStringEncodingEUC_KR; break;
	case 950: encoding = kCFStringEncodingBig5; break;
	case 1250: encoding = kCFStringEncodingWindowsLatin2; break;
	case 1251: encoding = kCFStringEncodingWindowsCyrillic; break;
	case 1252: encoding = kCFStringEncodingWindowsLatin1; break;
	case 1253: encoding = kCFStringEncodingWindowsGreek; break;
	case 1254: encoding = kCFStringEncodingWindowsLatin5; break;
	case 1255: encoding = kCFStringEncodingWindowsHebrew; break;
	case 1256: encoding = kCFStringEncodingWindowsArabic; break;
	case 1257: encoding = kCFStringEncodingWindowsBalticRim; break;
	case 1258: encoding = kCFStringEncodingWindowsVietnamese; break;
	case 874: encoding = kCFStringEncodingISOLatinThai; break;
	case 0: default: encoding = kCFStringEncodingISOLatin1; break;
	}
	const CFIndex usedBufLen = patternLen * 2 + 8;
	std::wstring ws(usedBufLen, L'\0');
	const CFStringRef s = CFStringCreateWithBytes(kCFAllocatorDefault,
		reinterpret_cast<const UInt8 *>(pattern), patternLen, encoding, false);
	if (s) {
		const CFIndex charCount = CFStringGetLength(s);
		CFStringGetCharacters(s, CFRangeMake(0, charCount),
			reinterpret_cast<UniChar *>(ws.data()));
		ws.resize(charCount);
		CFRelease(s);
	} else {
		ws.clear();
