...
#else
	// macOS port: 记录到期时间戳，IdleTaskTimeExpired() 用 chrono 比对
	idleTaskDeadline = std::chrono::steady_clock::now() + std::chrono::milliseconds(milliseconds);
#endif
}
...
#else
	return std::chrono::steady_clock::now() >= idleTaskDeadline;
#endif
}
